    ...
    try {
      ...
      if (allNFTs.length > 0) {
        for (const nft of allNFTs) {
          ...
          await db.query(`
            INSERT INTO venfts
            (token_id, owner, balance_raw, balance_formatted, unlock_timestamp, unlock_date, is_locked, lock_end_timestamp, snapshot_time)
            VALUES (${nft.tokenId}, '${nft.owner}', '${nft.balance}', ${balanceFormatted}, ${nft.lockedEnd}, '${unlockDate}', ${isLocked}, ${nft.lockedEnd}, CURRENT_TIMESTAMP)
          `);
        }
      }
