...
WITH daily_supply AS (
  SELECT
    ...
    SUM(CAST(balance_raw AS DOUBLE) / 1e18) as total_supply_value, -- Renamed for clarity in CTE
    COUNT(*) as total_nfts,
    COUNT(DISTINCT owner) as unique_holders,
    AVG(CAST(balance_raw AS DOUBLE) / 1e18) as avg_voting_power,
    MAX(CAST(balance_raw AS DOUBLE) / 1e18) as max_voting_power,
    MIN(CAST(balance_raw AS DOUBLE) / 1e18) as min_voting_power
  FROM venfts
